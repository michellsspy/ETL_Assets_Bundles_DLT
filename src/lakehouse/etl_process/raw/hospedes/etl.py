...
def create_table_if_not_exists(spark: SparkSession, table_name: str, schema: StructType):
    ...
    spark.sql(f"""
        ...
        USING DELTA
        COMMENT 'Tabela da camada Raw (hospedes) criada via job PySpark.'
    """)
...
if __name__ == "__main__":
    print("Iniciando o Job de Ingestão da Camada Raw para 'Hospedes'...")
    
    # Configuração específica desta tabela
    SOURCE_TABLE_NAME = "dev.transient.source_hospedes"
    TARGET_TABLE_NAME = "dev.raw.hospedes"
    SCHEMA = schema_raw_hospedes
    PRIMARY_KEYS = ["hospede_id"]
    
    spark_session = SparkSession.builder.appName("RawIngestion_Hospedes").getOrCreate()
    ...
    print("Job de Ingestão de 'Hospedes' concluído.")