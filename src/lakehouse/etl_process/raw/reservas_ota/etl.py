...
def create_table_if_not_exists(spark: SparkSession, table_name: str, schema: StructType):
    ...
    spark.sql(f"""
        ...
        USING DELTA
        COMMENT 'Tabela da camada Raw (reserva_ota) criada via job PySpark.'
    """)
...
if __name__ == "__main__":
    print("Iniciando o Job de Ingestão da Camada Raw para 'Reserva OTA'...")
    
    # Configuração específica desta tabela
    SOURCE_TABLE_NAME = "dev.transient.source_reserva_ota"
    TARGET_TABLE_NAME = "dev.raw.reserva_ota"
    SCHEMA = schema_raw_reserva_ota
    PRIMARY_KEYS = ["ota_reserva_id"]
    
    spark_session = SparkSession.builder.appName("RawIngestion_ReservaOTA").getOrCreate()
    ...
    print("Job de Ingestão de 'Reserva OTA' concluído.")