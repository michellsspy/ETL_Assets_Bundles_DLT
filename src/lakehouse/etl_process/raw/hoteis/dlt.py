...
from etl_assets_bundles_dlt.lakehouse.etl_process.raw.hoteis.schema import schema_raw_hoteis
...
TARGET_TABLE_DLT_NAME = "raw_hoteis" # Nome lógico usado no DLT
...
def hoteis_transient_stream():
    """
    Prepara o stream de dados da origem (source_hoteis).
...
dlt.apply_changes(
    target = TARGET_TABLE, # O nome completo da tabela final (ex: dev.raw.hoteis)
    source = "hoteis_transient_stream", # O nome da tabela DLT temporária acima
    keys = ["hotel_id"], # A Chave Primária para o MERGE
