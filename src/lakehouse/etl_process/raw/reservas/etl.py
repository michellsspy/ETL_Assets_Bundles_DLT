...
def create_table_if_not_exists(spark: SparkSession, table_name: str, schema: StructType):
    ...
    spark.sql(f"""
        ...
        USING DELTA
        COMMENT 'Tabela da camada Raw (reservas) criada via job PySpark.'
    """)
...
if __name__ == "__main__":
    print("Iniciando o Job de Ingestão da Camada Raw para 'Reservas'...")
    
    # Configuração específica desta tabela
    SOURCE_TABLE_NAME = "dev.transient.source_reservas"
    TARGET_TABLE_NAME = "dev.raw.reservas"
    SCHEMA = schema_raw_reservas
    PRIMARY_KEYS = ["reserva_id"]
    
    spark_session = SparkSession.builder.appName("RawIngestion_Reservas").getOrCreate()
    ...
    print("Job de Ingestão de 'Reservas' concluído.")