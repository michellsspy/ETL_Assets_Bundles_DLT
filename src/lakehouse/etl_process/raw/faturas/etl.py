...
def create_table_if_not_exists(spark: SparkSession, table_name: str, schema: StructType):
    ...
    spark.sql(f"""
        ...
        USING DELTA
        COMMENT 'Tabela da camada Raw (faturas) criada via job PySpark.'
    """)
...
if __name__ == "__main__":
    print("Iniciando o Job de Ingestão da Camada Raw para 'Faturas'...")
    
    # Configuração específica desta tabela
    SOURCE_TABLE_NAME = "dev.transient.source_faturas"
    TARGET_TABLE_NAME = "dev.raw.faturas"
    SCHEMA = schema_raw_faturas
    PRIMARY_KEYS = ["fatura_id"]
    
    spark_session = SparkSession.builder.appName("RawIngestion_Faturas").getOrCreate()
    ...
    print("Job de Ingestão de 'Faturas' concluído.")