...
def create_table_if_not_exists(spark: SparkSession, table_name: str, schema: StructType):
    ...
    spark.sql(f"""
        ...
        USING DELTA
        COMMENT 'Tabela da camada Raw (quartos) criada via job PySpark.'
    """)
...
if __name__ == "__main__":
    print("Iniciando o Job de Ingestão da Camada Raw para 'Quartos'...")
    
    # Configuração específica desta tabela
    SOURCE_TABLE_NAME = "dev.transient.source_quartos"
    TARGET_TABLE_NAME = "dev.raw.quartos"
    SCHEMA = schema_raw_quartos
    PRIMARY_KEYS = ["quarto_id"]
    
    spark_session = SparkSession.builder.appName("RawIngestion_Quartos").getOrCreate()
    ...
    print("Job de Ingestão de 'Quartos' concluído.")