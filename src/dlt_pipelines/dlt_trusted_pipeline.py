...
# Implementada com SCD Type 1 usando apply_changes e o padrão de 2 funções
...
def hospedes_raw_stream():
    ...
    return df_raw.select(
        ...
        col("data_ultima_hospedagem").cast("DATE"), col("total_hospedagens").cast("INT"), col("update_date")
    ).withColumn("data_carga_trusted", current_timestamp())
...
def consumos_raw_stream():
    ...
    return df_raw.select(
        ...
        col("local_consumo").cast("STRING"), col("funcionario_responsavel").cast("STRING"), col("update_date")
    ).withColumn("data_carga_trusted", current_timestamp())
...
def faturas_raw_stream():
    ...
    return df_raw.select(
        ...
        col("taxa_servico").cast("DOUBLE"), col("numero_transacao").cast("STRING"), col("update_date")
    ).withColumn("data_carga_trusted", current_timestamp())
...
def reservas_ota_raw_stream():
    ...
    return df_raw.select(
        ...
        col("valor_liquido_recebido").cast("DOUBLE"), col("ota_solicitacoes_especificas").cast("STRING"), col("update_date")
    ).withColumn("data_carga_trusted", current_timestamp())
